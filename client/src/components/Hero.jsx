  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        textAlign:'center', padding:'80px 32px 0',
        ...
        <h1 className="groovy-title" style={{
          fontSize:'clamp(64px,12vw,120px)', color:'#fff',
          textShadow:'6px 6px 0 #1a1630, 8px 8px 0 #b06dff66', marginBottom:4,
        }}>Feel the</h1>
        <h1 className="groovy-title" style={{
          fontSize:'clamp(64px,12vw,120px)',
          background:'linear-gradient(135deg,#FFD93D,#FF6FB7,#b06dff)',
          WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent',
          marginBottom:24, filter:'drop-shadow(8px 8px 0 #1a163044)',
        }}>Music ✦</h1>
        <p style={{
          fontSize:'clamp(16px,2.2vw,20px)', color:'#ffffffcc',
          lineHeight:1.7, maxWidth:560, margin:'0 auto 40px', fontWeight:600,
        }}>
          Tell us how you're feeling in any messy, honest, beautiful way
          and we'll match you with songs that <em>get it</em>.
        </p>
        <div style={{ display:'flex', gap:16, justifyContent:'center', flexWrap:'wrap', pointerEvents:'all' }}>
          <button
            onClick={() => {
              const max = scrollRef.current.scrollHeight - scrollRef.current.clientHeight
              scrollRef.current.scrollTo({ top: max * (2/SCENES + 0.01), behavior:'smooth' })
            }}
            className="retro-btn"
            style={{ background:'#FFD93D', color:'#0d0b14', fontSize:18, padding:'16px 38px' }}
          >
            Recommend me songs
          </button>
          <button
            onClick={() => {
              const max = scrollRef.current.scrollHeight - scrollRef.current.clientHeight
              scrollRef.current.scrollTo({ top: max * (1/SCENES + 0.01), behavior:'smooth' })
            }}
            className="retro-btn"
            style={{ background:'transparent', color:'#fff', borderColor:'#ffffff55', boxShadow:'4px 4px 0 #ffffff22' }}
          >
            Meet your moods ↓
          </button>
        </div>
      ...
      <div style={{
        ...
        color:'#ffffff55', fontSize:12, fontFamily:"'Space Mono',monospace",
        animation:'shimmer 2s infinite', opacity: hintOp, pointerEvents:'none',
      }}>
        <span>scroll to explore</span>
        <div style={{ width:1, height:40, background:'linear-gradient(to bottom,#fff5,transparent)' }}/>
      </div>
    </div>
  )
}
